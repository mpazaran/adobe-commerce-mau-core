...
namespace Freedom\Core\Api\SearchCriteria\CollectionProcessor;
...
class FilterProcessor implements CollectionProcessorInterface
{
    ...
    private function addFilterGroupToCollection(
        ...
        foreach ($filterGroup->getFilters() as $filter) {
            if($filter->getField() instanceof Zend_Db_Expr) {
                $fields[] = $filter->getField();
                $conditions[] = [$filter->getConditionType() ? $filter->getConditionType() : 'eq' => $filter->getValue()];
                continue;
            }
            $isApplied = false;
